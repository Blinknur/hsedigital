...
interface ComplianceMapViewProps {
    stationsWithCompliance: { station: Station; compliance: number }[];
}

const getStatusColor = (compliance: number): string => {
    if (compliance === -1) return 'bg-slate-400'; // No data
    if (compliance < 80) return 'bg-red-500';
    if (compliance < 95) return 'bg-amber-500';
    return 'bg-green-500';
};
...
const ComplianceMapView: React.FC<ComplianceMapViewProps> = ({ stationsWithCompliance }) => {
    return (
        <div>
            <div className="relative w-full h-48 bg-slate-200 rounded-lg overflow-hidden border border-slate-300">
                ...
                {stationsWithCompliance.map(({ station, compliance }) => {
                    ...
                    return (
                        ...
                        >
                             <div className={`w-3 h-3 rounded-full border-2 border-white shadow-md ${getStatusColor(compliance)} transition-transform group-hover:scale-125`}></div>
                             <div className="absolute bottom-full mb-3 w-max max-w-xs p-2 text-xs text-white bg-slate-800 rounded-md opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none -translate-x-1/2 left-1/2">
                                ...
                                <p>Compliance: {compliance === -1 ? 'No Data' : `${compliance.toFixed(1)}%`}</p>
            ...
            <div className="flex justify-around mt-2 text-xs text-slate-600">
                <span className="flex items-center"><span className="w-2.5 h-2.5 rounded-full bg-green-500 mr-1.5"></span>&gt; 95%</span>
                <span className="flex items-center"><span className="w-2.5 h-2.5 rounded-full bg-amber-500 mr-1.5"></span>80-95%</span>
                <span className="flex items-center"><span className="w-2.5 h-2.5 rounded-full bg-red-500 mr-1.5"></span>&lt; 80%</span>
                <span className="flex items-center"><span className="w-2.5 h-2.5 rounded-full bg-slate-400 mr-1.5"></span>No Data</span>
            </div>
        </div>
    );
};
